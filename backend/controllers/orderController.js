...
export const createOrder = async (req, res) => {
  try {
    ...
    if (paymentMethod === "online") {
      ...
      if (seat) {
        await Seat.findByIdAndUpdate(seat, { status: "occupied" })
      }
...
export const updateAnyOrder = async (req, res) => {
  try {
    ...
    // If status changed to "delivered" or "cancelled", free the seat
    ...
    if (
      ...
      await Seat.findByIdAndUpdate(order.seat, { status: "available" })
    }
